...
func (s *TransactionDBTestSuite) SetupSuite() {
	...
	client, err := entity.NewClient("João", "teste@teste.com")
	s.Nil(err)
	s.client = client

	client2, err := entity.NewClient("John", "testeII@teste.com")
	s.Nil(err)
	s.client2 = client2
	//creating accounts
...
func (s *TransactionDBTestSuite) TestCreate() {
	transaction, err := entity.NewTransaction(s.accountFrom, s.accountTo, 100) //Criei a transação
	s.Nil(err) 
	err = s.transactionDb.Create(transaction) //Salvei a transação
	s.Nil(err)
}